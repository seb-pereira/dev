...
SELENIUM_DIR=servers/selenium
CHROMEDRIVER_DIR=${SELENIUM_DIR}/chrome-driver
LOG_FILE=logs/selenium.log

echo "Starting selenium server..." | tee ${LOG_FILE}
${JAVA_BIN} -jar ${SELENIUM_DIR}/selenium-server-standalone.jar \
	-debug \
	-port 4444 \
	-Dwebdriver.chrome.driver=${CHROMEDRIVER_DIR}/chromedriver | tee -a ${LOG_FILE}
